...
GTEST_TEST(ParticlesClassTest, TestReorder) {
    ...
    EXPECT_TRUE(CompareMatrices(particles.get_position(0), pos1, TOLERANCE));
    EXPECT_TRUE(CompareMatrices(particles.get_velocity(0), vel1, TOLERANCE));
    EXPECT_EQ(particles.get_mass(0), mass1);
    EXPECT_EQ(particles.get_reference_volume(0), vol1);
    EXPECT_TRUE(CompareMatrices(particles.get_deformation_gradient(0), F1,
                TOLERANCE));
...
GTEST_TEST(ParticlesClassTest, TestAdvect) {
    Vector3<double> pos1 = {1.0, 2.0, 3.0};
    Vector3<double> vel1 = {-1.0, -2.0, -3.0};
    Vector3<double> pos2 = {3.0, -1.0, 6.0};
    Vector3<double> vel2 = {-9.0, 8.0, -2.0};
    Particles particles = Particles();
    particles.AddParticle(pos1, vel1, 1.0, 1.0,
                          Matrix3<double>::Zero(), Matrix3<double>::Zero());
    particles.AddParticle(pos2, vel2, 1.0, 1.0,
                          Matrix3<double>::Zero(), Matrix3<double>::Zero());
    ...
    EXPECT_TRUE(CompareMatrices(particles.get_position(0),
                                Vector3<double>(0.7, 1.4, 2.1), TOLERANCE));
    EXPECT_TRUE(CompareMatrices(particles.get_position(1),
                                Vector3<double>(0.3, 1.4, 5.4), TOLERANCE));
}

GTEST_TEST(ParticlesClassTest, TestUpdateKirchhoffStress) {
    CorotatedModel corotated_model = CorotatedModel(5.0, 0.25);

    const Matrix3<double> R =
        math::RotationMatrix<double>
            (math::RollPitchYaw<double>(M_PI/2.0, M_PI, 3.0*M_PI/2.0)).matrix();
    ...
    Particles particles = Particles();
    particles.AddParticle(Vector3<double>(0.0, 0.0, 0.0),
